...
<button className={`btn btn-sm m-1 ${props.contact.isFavourite?
...
<button className="btn btn-primary btn-sm m-1">
  <i className="bi bi-pencil" onClick={()=> props.updatedClick(props.contact)} style={{fontSize:"1rem"}}></i>
</button>
<button className="btn btn-danger btn-sm m-1">
  <i className="bi bi-trash-fill" style={{fontSize:"1rem"}}
  onClick={() =>props.deletecontact(props.contact.id)}></i>
</button>
        </div>
      </div>
    );
